/*
Controller for interface "People" (context: "ProjectAdministration"). Generated code, edit with care.
*/
AmpersandApp.controller('PeopleController', function ($scope, $rootScope, $route, $routeParams, Restangular, $location, $timeout, $localStorage, $sessionStorage) {
	$scope.interfaceName = "People";
	$scope.interfaceLabel = "People";
	...
	$scope.val['People'] = new Array();
	
	if($routeParams['new']){
		$scope.loadingInterface.push(
			$scope.srcAtom.all('People').post({})
				.then(function(data) { // POST
				$rootScope.updateNotifications(data.notifications);
				$scope.val['People'].push(Restangular.restangularizeElement($scope.srcAtom, data.content, 'People')); // Add to collection
				showHideButtons(data.invariantRulesHold, data.requestType, data.content.id);
			})
		);
	}else{
	    $scope.loadingInterface.push(
	    	$scope.srcAtom.all('People').getList().then(function(data){
	    		if($.isEmptyObject(data.plain())){
	    			$rootScope.addInfo('No results found');
	    		}else{
	    			$scope.val['People'] = data;
	    			$scope.resourceLabel = $scope.val['People'][0]['@label'];
	    		}
	    	})
	    );
    }
	...
	$scope.addNewResource = function (prepend){
		...
		$scope.loadingResources['_new_'].push(
			$scope.srcAtom.all('People')
				.post({})
				.then(function(data){ // POST
					$rootScope.updateNotifications(data.notifications);
					if(prepend) $scope.val['People'].unshift(Restangular.restangularizeElement($scope.srcAtom, data.content, 'People')); // Add to collection
					else $scope.val['People'].push(Restangular.restangularizeElement($scope.srcAtom, data.content, 'People')); // Add to collection
					showHideButtons(data.invariantRulesHold, data.requestType, data.content.id);
	...
	$scope.deleteResource = function (resourceId){
		if(confirm('Are you sure?')){
			var resourceIndex = _getResourceIndex(resourceId, $scope.val['People']);
			
			// myPromise is used for busy indicator
			$scope.loadingResources[resourceId] = new Array();
			$scope.loadingResources[resourceId].push(
				$scope.val['People'][resourceIndex]
					.remove({ 'requestType' : 'promise'})
					.then(function(data){
						$rootScope.updateNotifications(data.notifications);
						$scope.val['People'].splice(resourceIndex, 1); // remove from array
					})
			);
		}
	}
	...
	$scope.put = function(resourceId, requestType){
		...
		if(location['new']){
			$scope.loadingResources[resourceId].push(
				$scope.srcAtom.all('People')
				.post($scope.val['People'][resourceIndex].plain(), {'requestType' : requestType})
				.then(function(data) { // POST
					$rootScope.updateNotifications(data.notifications);
					$scope.val['People'][resourceIndex] = $.extend($scope.val['People'][resourceIndex], data.content);
					showHideButtons(data.invariantRulesHold, data.requestType, data.content.id);
					
					if(data.invariantRulesHold && data.requestType == 'promise'){
						// Resource is posted, change url
						$location.url('/People/' + data.content.id);
					}
				})
			);
		// else => PUT
		}else{
			$scope.loadingResources[resourceId].push( $scope.val['People'][resourceIndex]
		...
		$scope.loadingResources[resourceId].push(
			$scope.val['People'][resourceIndex]
				...
				.then(function(data){
					...
					$scope.val['People'][resourceIndex] = $.extend($scope.val['People'][resourceIndex], data.plain());
	...
	$scope.patch = function(resourceId){
		var resourceIndex = _getResourceIndex(resourceId, $scope.val['People']);
		patches = diff($scope.initialVal['People'][resourceIndex], $scope.val['People'][resourceIndex]) // determine patches
