...
Requirement engineers similiar as Dr. Joosten, are responsible of translating 
business 
...
the client -- in a fully automatic, provably correct way (\cite{applyAlgebra}). 
...
\subsection{End Users: Process Innovators}
...
This criteria represents how important a functional requirement is to the 
customer, customer satisfaction indicates how satisfied our client would be if 
we completed this requirement while customer dissatisfaction represents how 
unhappy our client would be if the requirement is left unsatisfied.
	\begin {itemize} 
	\item[$\rightarrow$] low: the client cares very little if this requirement 
	is satisfied and it is completely optional
	\item[$\rightarrow$] medium-low: the client believes it would be useful to 
	have but is not of any importance
	\item[$\rightarrow$] medium: the client would be happy if it were fulfilled 
	but is not fundamental
	\item[$\rightarrow$] 4 - High: requirement is instrumental to meeting 
	client's satisfaction, but not an absolute necessity
		\item[$\rightarrow$] 5 - Highest: requirement is of the highest 
		importance to the client and must be fulfilled, else the client will be 
		very dissatisfied.
\end{itemize} 
Conflicts: 
	\begin {itemize} 
