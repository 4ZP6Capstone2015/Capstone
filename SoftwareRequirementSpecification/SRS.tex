...
\begin{figure}
	...
	\begin{tabular}{ |m{4cm}|m{6cm}|m{4cm}|  }
		...
		19/10/2015 & EFA SRS version 1  & 19/10/2015 \\
...
chance Dan hasn't read our new problem statement so he doesn't necessarily have 
...
%% EFA provides the ability to ensure no contradictions are found 
%% within the design by automatically fixing inconsistency that may arise. 
%% Irrelevant.... 
\edcomm{JG}{What is irrelevant? I welcome help when writing this.. but if im 
not given criteria and I have no idea what you're saying is irrelevant; thats 
not going to result in change}
...
the Architect matches any building regulations that must be met
\ds{``met"}. \\ \indent
Regardless of 
...
\ds{Break up this wall of text into paragraphs.}

\ds{This entire section (1.2) has been unnecessarily verbose. A simple explanation
	of who the stakeholder is and why they hold a stake would be enough.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Chapter 2: Project Constraints %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Project Constraints}\label{ch:Constraints}
The main constraints of this project is time and the necessary use of Haskell. 
The time allotted is 8 months. This is the maximum amount of we have to build 
and effectively test our system. 
The other restraint is the use of coding language which must be Haskell as it 
is the language choosen by the original designers,and thus any source code that 
is added must be written in Haskell. 
...
recommended way
\ds{Convenient, but not the only way? Then is it really a constraint?
Is there a specific reason you \emph{must} use XAMPP?}
...
however it will not be automatically detected by Ampersand to use as part of 
its system.
...
\item[ECA] Stands for Event-Condition Action. The rule structure used for data
 ...
 %TODO{JG}: bold or italicize words in definitions
...
\ds{A lot of these are far too specific. I do not need to know implementation
details to understand the abstract requirements.}
...
specification of the services, and a function point analysis
	...
	used to prescribe properties of a system to be built; and requirement 
	describes an explicit or implicit need of users input}
...
can't even tell if Ampersand is a system, 
...
\ds{Why is this section here and empty?}
...
\ds{``artifacts"} generated by Ampersand; instead
...
documentation and training/services  \\ \hline[6pt] 
...
\ds{``respond" -- I'm no longer looking at spelling or grammar. It is up to you to 
find the rest of the mistakes (if any) before the next revision is submitted.}
...
what those problems may be are unforseeable at this stage of development. 
...
\end{itemize}
\edcomm{JG}{To YT: I'm sorry I used bullet points, but I needed more before 
writting this out and its 4:30 in the morning, I'm not sure you're available}
...
%% YT: I got rid of 'Conflict' altogether from all the reqs. 
...
%% 	\item[$\rightarrow$] major: severe system conflict with Ampersand and 
%% 	requires alternative implementation
%% 	\item[$\rightarrow$] severe: requires immediate attention and product 
%% 	cannot function due to severe system conflict; needs over haul for proceed; 
%% 	this may constitute a dead end.
...
\\	Fit Criterion & Able to compile using GHC %% --haskell function  
\\	Priority & 5 - Highest
\\	Supporting Materials & Ampersand Documentation (Rule Based Design \cite{RBD}) 
\vspace{12pt}
\end{tabularx}
}%
\ds{This is a constraint, not a functional requirement.}
{\setlength{\tabcolsep}{12pt} %% Default is 6
\begin{tabularx}{\textwidth}{>{\bfseries}C{3cm}X}
Requirement & S2 \\ 
\midrule
...
\\	Rationale & Cohesion of new product into existing system; 
                    necessary for EFA to be added to source code of Ampersand 
\\	Originator & Client/Designers 
\\	Fit Criterion & Able to submit pull request on GitHub %% --haskell function  
\\	Priority & 5 - Highest
\\	Supporting Materials & Ampersand Documentation (Rule Based Design \cite {RBD}) 
\vspace{12pt}
\end{tabularx}
...
\endhead
	...
	%%TODO ref to this? JG: your choice?
\vspace{12pt}
\end{tabularx}
}
...
\ds{You just explained what NFRs are, but did not contribute anything. Perhaps
 just introduce the diagram.}
\edcomm{JG}{How do I pull the tree up to the first page of non-functional?? 
*facepalm* \vspace{-infinity} doesnt work}
...
the action performed by EFA within Ampersand without knowledge of its implementation. 

EFA will affect the usability of Ampersand mainly by increasing the amount of
automation in Ampersand. However, automation is a dangerous thing, especially in
terms of usability: if there is too much automation and not enough feedback
given to the user, then the user will likely be very confused about how her output
artifacts were derived from the input. Therefore, 

%% YT: Similies don't belong in a technical document...
%% , it would be similar to someone attempting to distinguish a person’s hand as 
%% a separate entity from that person. 

%% YT: This part doesn't seem right... we want full automation. Yes, we want to
%% propogate *information* to the user, but not require that the user give
%% additional input.

%% EFA may not look very personable, or user friendly, but it 
...
This product can be used by any individual who is willing to dedicate the time 
to learning the basics of the Ampersand model and wishes to design an 
information system. The individual must be computer literate and able to debug 
simple software programs.  
%% , a normal individual may take anywhere from a week to a month. 
%% YT: Can't say this without proof...
An engineer who has a strong background in mathematical logic or minor
experience in programming will be able to learn to use Ampersand efficiently
must faster than one who only works with design elements; however, the Ampersand
method can be taught to anyone with sufficient background in requirements
engineering (\cite[p.~1]{derFun}). 

The most pronounced problem concerning Ampersand’s usability is that it’s
limited to two languages: English and Dutch. Those who are not fluent in
neither of these languages will have great difficulty learning the Ampersand
system. However, this is mainly related to the user base and popularity of
Ampersand; the majority of its users are its designers, and their primary
languages are these two.

%% YT: Removing this... I tracked down the reference to CC, this is very
%% irrelevant and its mention here is just confusing.
...
%% diverging opinions on the same matter. 
Once the prototype is generated, it can be presented to 
...
\ds{So what are the usability and humanity requirements? All you did was explain
what constitutes one of these requirements and some of Ampersand's current system.}

\section{Understandability}
\paragraph*{}
What EFA does is simple to understand: it fixes logical inconsistencies with minimal, if any,
input from the user. It does this by looking for contradictions, or violations. 

An overly simplistic, but accurate,
example would be: there is a red chair, there is a blue chair, but there is only one chair. That chair cannot 
...
then one of the colours statements is erased. 
It may pick a chair colour depending on another criteria that comes from another
part of the information system. For example, if majority of customers' favourite colour is red,
then most individuals who may use the
chair would prefer it to be red rather than blue. Ampersand has the capability
of tracking relations between criteria like these. %% Does it? Is this an accurate descrition of the system?
...
\paragraph*{}
Ampersand can efficiently process a large amount of information in a short amount of time; and the 
process that EFA requires will be added onto Ampersand’s process time. The time it 
...
process is terminated if the running time exceeds this threshold, for a simple model.
...
products proceed into the production stage (\cite[153]{RBD}). However, our contribution will

include the propogation of proofs as well as feedback to the user. These proofs
...
those system are limited to Windows OS 7 and up. %% What, you can't build Ampersand on linux? It's a
%% cabal package... it should work. This seems dubious, please confirm.
There are very few physical conditions required to 
maintain EFA; they are the basic conditions required to keep most computer system running. 
...
\edcomm{YT}{Not necessary to say what the number is. The requirement isn't precise enough. }
...
participating on the same project. In order to meet the maintainability requirements, EFA must make sure that
each specification is traceable back to the motivation for its existence 
...
individuals and businesses with any part of Ampersand, providing a well structured error message will allow to user to trace back and solve the issue without external assistance. Moreover, EFA documentation will have various examples for the user to learn from.
\edcomm{who}{Part of the reason that non-functional requirements as essential to EFA concerns the 
