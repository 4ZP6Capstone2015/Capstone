...
information about ECA rules is propogated
\ds{``propagated"}
through the pileline 
\ds{``pipeline"?} 
of
...
\ds{This paragraph seemed out of place}
...
the Architect matches any building regulations that must be meet
\ds{``met"}.
Regardless of 
...
\item [Natural language] Language written in a manner similair
\ds{``similar"}
...
omits many of the uses of the design artifcats
\ds{``artifacts"}
...
\ds{This table needs to have its columns adjusted. The description should be the widest column.}
...
Business requirements & Object & Important constraints seperated
\ds{``separated"}
into logical units, see section \ref{sec:Naming} \\[6pt] 
...
Reevaluate
\ds{``Re-evaluate"}
requirements & Process & Requirements deemed not valid and must be reevaluated
\ds{``re-evaluated"}
\\[6pt]
...
Database Management Systems. (\cite[p.202]{ECA})
...
Ampersand is truly up to the user. 
