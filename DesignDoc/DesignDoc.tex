...
is the \lstinline{Alias} function, which is a primitive syntactic constructor (not a named entity)
...
each element of the row, as well as their ordering). The single function \lstinline{primSQL}
...
\begin{tikzpicture}
  ...
  \begin{class}[text width=22cm]{Equality}{17,5}
    ...
    \hsfunc{cong2 : f :\~: g -> a :\~: a' -> b :\~: b' -> f a b :\~: g a' b'}
    \hsfunc{cong3 : f :\~: g -> a :\~: a' -> b :\~: b' -> c :\~: c' -> f a b c :\~: g a' b' c'}
    ...
    \hsfunc{($\#$>>) :: Exists p -> (forall x -> p x -> r) -> r}
    ...
    %% Why is this particular defintion off by about 2pt? Very strange...
    \hstype[-2pt]{triviallyTrue : Not (Not ())}

    \hstype{mapNeg : (NFData a, NFData b) => (b -> a) -> Not a -> Not b}
    ...
    \hstype{DecEq : k -> k -> Type = $\lambda$ a b$.$ Dec (a :\~: b)}

    \hsfunc{mapDec : (p -> q) -> (Not p -> Not q) -> Dec p -> Dec q}

    \hsfunc{liftDec2 :: Dec p -> Dec q -> (p -> q -> r) -> (Not p -> Not r) -> (Not q -> Not r) -> Dec r}
