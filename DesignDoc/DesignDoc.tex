...
may be dropped when the kind $A$ is clear from the context: \lstinline!forall (x : A) -> P x!,
\lstinline!forall x -> P x!, and \lstinline!x -> P x! all denote the type \texttt{forall (x :: A) . P x}.
...
\begin{tikzpicture}
  ...
  \begin{package}{Module}
    ...
    \begin{class}[text width=10cm]{Submodule$_1$}{0,-7}
...
\begin{tikzpicture}
  ...
  \begin{class}[text width=30cm]{TypedSQLStatement}{10,18}
    ...
    \hstypectr{NewRef : IsScalarType a \~ True -> SQLTypeS a -> Maybe String -> Maybe (SQLVal a) -> SQLStatement (SQLRef a)}
