...
Haskell ``base'' package (which is the core libraries of Haskell). In general,
...
to work with a modification to Ampersand, only one EFA module -- ECA2SQL -- will generally need 
to be modified. 
...
\begin{tikzpicture}
  ...
  \renewcommand{\umlfillcolor}{grey}
  \begin{class}[text width=3cm]{base}{0,-13.5}
      \hsfunc{$\ldots$}
  \end{class}

  \renewcommand{\umlfillcolor}{grey}
  \begin{class}[text width=6cm]{wl-pprint}{-3.4,-8}
    \hstype{Doc : Type} 
  ...
  \renewcommand{\umlfillcolor}{grey}
  \begin{class}[text width=5cm]{simple-sql-parser}{-10,-8}
      \hsfunc{$\ldots$}
  \end{class}
...
precisely to entities defined by MySQL. Only a few
...
\begin{tikzpicture}
  ...
  \begin{package}{Base}
    ...
    \renewcommand{\umlfillcolor}{grey}
    \begin{class}[text width=10cm]{Primitive}{0,3}

      \hstype{Nat : Kind} 
      \hstype{Constraint : Kind} 
      \hstype{Type : Kind} 
      \hstype{class $\,\,\text{KnownNat}$ : Nat -> Constraint where $\ldots$} 
      \hstype{Symbol : Kind}
      \hstype{class KnownSymbol : Nat -> Constraint where $\ldots$}  
      \hstype{(\~) : Type -> Type -> Constraint} 

    \end{class}

    \begin{class}[text width=10cm]{List}{0,5}
      \hstype{$\ldots$}
    \end{class}

    \begin{class}[text width=10cm]{Maybe}{0,7}
      \hstype{$\ldots$}
    \end{class}
...
\end{tikzpicture}
}}\caption{Module diagram for \texttt{base} package} \label{fig:basePkg}
\end{figure}
