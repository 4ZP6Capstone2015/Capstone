...
\noindent
\subsection*{Description of \edchange{WK}{Dependant}{Dependent} types}
...
looks like a 
real Haskell type \edchange{WK}{-}{---}\edcomm{WK}{always!} in Haskell, the syntax would be \texttt{forall (x :: A) . 
B}. However, 
...
\edcomm{WK}{Having an English-sentence-end-``.'' between two Haskell
fragments at the end of the next sentence makes them look like a
single, \textbf{unreadable} Haskell fragment! \textbf{NEVER} do ``math-punctuation-math''!}
...
where $Ty$ \edcomm{WK}{Inconsistent font for \lstinline|Ty|!} is 
the name
...
guide~\cite{ghcUserGuide}\edcomm{WK}{Use ``key = \{GHC\}'' in BibTeX}. While
