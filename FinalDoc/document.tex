...
on business process requirements. \edchange{WK}{At times, logical 
discrepancies arise when system changes occur which violate the 
restrictions set forth by the user.%
...
When a \edchange{WK}{system}{invariant} violation occurs, one of two 
...
discarded. The
\edinsert{WK}{ultimate}
purpose of Event-Condition-Action rules for Ampersand (EFA) is to
replace
\eddelete{WK}{the}
Ampersand's \edchange{WK}{exec-engine}{``Exec-engine''}
which is \edinsert{WK}{currently} used to fix system violations.
\edchange{WK}{
Although the exec-engine appears to fix violations in the prototype, manual labour is 
required to fix system invariants inside of Ampersand.
}{However, the Exec-engine relies on manual formulation of repair
  actions in an error-prone setup that creates PHP-embedded SQL
  commands from string fragments, as opposed to using the
  automatically-generated Event-Condition-Action (ECA) rules
  that Ampersand already produces.}
The EFA project \edchange{WK}{was 
\edcomm{JG}{ proposed might not be the right word}
proposed to replace the exec-engine
}{aims to utlimately make the Exec-engine superfluous
}%edchange
by correctly translating \edinsert{WK}{the} ECA rules and 
...
provably correct \edcomm{WK}{This statement would require an axiomatic
  semantics for SQL and Hoare-logic proofs to be produced alongside
  your SQL generation --- something you never attempted.
A claim you can reasonably make is safety: Your generated SQL is by
construction syntax- and type-correct.}
  and has a high degree of modularity as it only relies on 
...
\edchange{WK}{This document acts as a comprehensive guide to EFA, the}{The} first chapter covers 
...
requirements.
The third \edchange{WK}{section}{chapter} focuses on design implementation and provides 
key information 
on EFA modules. In addition, it provides an outline of the AMMBR algorithm, 
which is used to generate ECA rules\edchange{WK: ???}{\ and}{, and of}
...
\href{https://github.com/AmpersandTarski}{Ampersand Tarski's Github }\edinsert{WK}{.}
...
\begin{description}
    ...
    \item [Ampersand] Ampersand is the name of this project\edcomm{WK}{`this project''???}.
