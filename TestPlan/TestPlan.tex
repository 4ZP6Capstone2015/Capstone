...
{PA clause executing ``All'' subclauses}  %% Test name
{???}  
{???}
{???}
...
\systemTest{T10}{EFA Mock Trial}
{Non-functional}
{First Week of February 2016 }
{N5}
\vspace{-12pt}\subsubsection*{Brief Explanation}
The design team shall create a mock database and fill it with relevant data. 
EFA will be tested to see if it is able to automatically fix data violations on 
a the mock database before any trials will take place in the real world. 
\vspace{-12pt}\subsubsection*{Input}
ECA Rules: \\
   ECA = \{  \\ \tabb \tabb ECA Trigger: if cast member does not have relevant 
   experience or 
...
\noindent
if $<$relevant experience$>$ in $<$this table$>$ \\
is under $<$this amount$>$, \\
remove $<$person$>$ from table.$<$applicants$>$
