...
{The input shall be a script written in ADL (abstract data language) in its initial state.}
{The output shall vary depending on the test type. \\ For example, the output for a quickcheck test shall be a success message, while on the other hand the output for a GUI test shall be the final state of the program.}
{A script shall be taken as input at which point EFA shall automate the correction of rule violations. This test shall uncover errors that occur in implementing requirements or design specification. This test is not concerned with the process works but with the results, furthermore, it focuses on functional requirements and normal behaviour. Dynamic analysis is used to examine the physical response from system variables that are not constant and change with time. }
